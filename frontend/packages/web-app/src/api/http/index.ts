...
class HttpClient {
  ...
  public init() {
    ...
    this.instance.interceptors.request.use((config: InternalRequestConfig) => {
      ...
      // InsForge JWT: prefer the SDK-managed session token, then fall back to
      // the legacy storage slot used by older rpa-auth flows.
      const token = getSessionToken() || sessionStorage.getItem('tokenValue')
