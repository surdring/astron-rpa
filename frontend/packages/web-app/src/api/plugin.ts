...
export async function getSupportBrowser() {
  const res = await rpaApi.http.get<{ browsers: string[] }>('/scheduler/browser/plugins/get_support', null, {
    toast: false,
  })
...
export function checkBrowerPlugin(browsers: string[]) {
  return rpaApi.http.post<Record<string, { installed: boolean, installed_version: string, latest: boolean, browser_installed: boolean }>>(
