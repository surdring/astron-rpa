...
export async function getComponentForm(params: {
  ...
  const info = context === 'get'
    ? (await rpaApi.robots.getEditComponentDetail({ componentId, robotId: processStore.project.id })).data
    : (await rpaApi.projects.getComponentDetail({ componentId })).data
...
export async function trackComponentUsageChange(operation: () => void | Promise<void>) {
  ...
  for (const key of addedKeys) {
    await rpaApi.robots.addComponentUse({
  ...
  for (const key of deletedKeys) {
    await rpaApi.robots.deleteComponentUse({
      robotId: useProcessStore().project.id,
      componentId: getComponentId(key),
    })
  }
