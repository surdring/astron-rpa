...
function handleAutoTranslate() {
  rpaApi.robots.getRobotEnglishName(formState.value.name).then((res) => {
    formState.value.english_name = res.data || ''
  })
}
function initData() {
  Promise.all([
    rpaApi.robots.getRobotLastIsExternalCall(props.record.robotId),
    crudApi.param.all({ robotId: props.record.robotId }),
  ]).then((res) => {
...
async function handleOk() {
  try {
    ...
    await rpaApi.robots.setRobotIsExternalCall({
      ...formState.value,
      project_id: props.record.robotId,
