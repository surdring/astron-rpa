...
export function useTaskEdit(taskId: string, handleRefresh: () => void, handleClose: () => void) {
  ...
  async function getTask() {
    rpaApi.tasks.getTaskInfo({ taskId }).then((res) => {
      if (res.data) {
  ...
  async function checkDuplicateTaskName(_rule, value) {
    ...
    if (isEdit.value)
      return Promise.resolve()
    const data = await rpaApi.tasks.isNameCopy({ name: value })
  ...
  function handleSave() {
    ...
    formRef.value
      ...
      .then(async () => {
        ...
        if (isEdit.value) {
          await rpaApi.tasks.updateTask(task)
        }
        else {
          await rpaApi.tasks.insertTask(task)
        }
