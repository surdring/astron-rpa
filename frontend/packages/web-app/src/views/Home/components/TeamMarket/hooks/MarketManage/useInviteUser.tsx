...
export function usePhoneInvite(marketId: string, type: string = 'invite', emit?: any) {
  ...
  const userListByPhone = debounce((keyword) => {
    ...
    const func = type === 'invite' ? rpaApi.market.getInviteUser : rpaApi.market.getTransferUser
...
export function useLinkInvite(marketId: string, emit?: any) {
  ...
  const generateLink = async () => {
    const res = await rpaApi.market.generateInviteLink({ marketId, expireType: formState.expireType })
    retInviteLink(res.data)
  }

  const resetLink = async () => {
    const res = await rpaApi.market.resetInviteLink({ marketId, expireType: formState.expireType })
    retInviteLink(res.data)
  }
