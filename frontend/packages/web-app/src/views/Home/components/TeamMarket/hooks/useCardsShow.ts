...
export function useCardsShow(emits) {
  ...
  async function handleDeleteApp(item) {
    ...
    await rpaApi.market.deleteApp({ appId, marketId })
  ...
  async function handleAppAchieve(e: Event, item) {
    ...
    let error = false
    try {
      const { data } = await rpaApi.market.canAchieveApp({ appId, marketId })
    ...
    if (needApplication) {
      useApplicationConfirm(t('market.insufficientPermissionNeedApplyConfirm'), () => {
        rpaApi.market.useApplication({ appId, marketId }).then(() => {
    ...
    const data = await rpaApi.market.getPushHistoryVersions(item)
