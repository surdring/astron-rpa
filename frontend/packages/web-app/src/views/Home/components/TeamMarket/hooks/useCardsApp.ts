...
export function useCardsApp() {
  ...
  async function getCardsData(params) {
    if (params.marketId) {
      const { total, records } = await rpaApi.market.getAppCards(params)
      // 获取初始化更新ID
  ...
  async function getMembersByTeam(marketId) {
    const { records } = await rpaApi.market.marketUserList({
      marketId,
      pageNo: 1,
      pageSize: 10000,
    })
  ...
  async function getAppCategory() {
    const res = await rpaApi.market.getAllClassification()
