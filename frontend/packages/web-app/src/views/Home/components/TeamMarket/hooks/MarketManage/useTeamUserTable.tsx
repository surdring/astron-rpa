...
export function useTeamUserTable() {
  ...
  const getUserList = async (params) => {
    const data = await rpaApi.market.marketUserList({
  ...
  const removeUser = ({ creatorId }) => {
    GlobalModal.confirm({
      ...
      okType: 'danger',
      onOk: () => {
        rpaApi.market.removeUserRole({ creatorId, marketId: activeMarket.value.marketId }).then((res) => {
          const { data } = res
          if (data) {
      ...
      const m = GlobalModal.confirm({
        ...
        onOk: () => {
          return new Promise((resolve, reject) => {
            ...
            rpaApi.market.inviteMarketUser({ marketId: activeMarket.value.marketId, userInfoList: inviteUsers.value })
              .then((res) => {
                if (res.data) {
  ...
  const leaveTeam = () => {
    GlobalModal.confirm({
      ...
      onOk: () => {
        rpaApi.market.leaveTeamMarket({ newOwner: '', marketId: activeMarket.value.marketId })
          .then(async (res) => {
            if (res.data) {
  ...
  const giveOwner = () => {
    ...
    GlobalModal.confirm({
      ...
      onOk: () => {
        ...
        rpaApi.market.leaveTeamMarket({ newOwner: newManager.value, marketId: activeMarket.value.marketId })
          .then(async (res) => {
            if (res.data) {
              message.success(t('common.operationSuccess'))
              await marketStore.refreshTeamList()
  ...
  const fireTeam = () => {
    ...
    GlobalModal.confirm({
      ...
      onOk: () => {
        ...
        rpaApi.market.dissolveTeamMarket({ marketName: teamName.value, marketId: activeMarket.value.marketId })
          .then(async (res) => {
            if (res.data) {
              message.success(t('common.operationSuccess'))
              await marketStore.refreshTeamList()
