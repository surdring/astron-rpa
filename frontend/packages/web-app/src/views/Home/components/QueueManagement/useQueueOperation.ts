...
export default function useQueueOperation() {
  ...
  async function getTableData(params) {
    const { data } = await rpaApi.tasks.getTaskQueueList(params)
  ...
  async function deleteQueueTask(record) {
    ...
    await rpaApi.tasks.removeTaskQueue({ unique_id: [record.unique_id] })
  ...
  const batchDelete = async () => {
    ...
    await rpaApi.tasks.removeTaskQueue({ unique_id: selectedRowKeys.value })
