...
export function useProjectOperate(
  ...
  async function createCopy(editObj: AnyObj) {
    if (userStore.currentTenant?.tenantType !== 'enterprise') {
      const res = await rpaApi.projects.checkProjectNum()
      if (!res.data) {
  ...
  function shareToMarket(editObj: AnyObj) {
    ...
    rpaApi.market.getTeams().then((data) => {
      $loading.close()
  ...
  async function handleDeleteProject(editObj: AnyObj) {
    ...
    const data = await rpaApi.projects.isInTask({ robotId })
    if (data) {
      ...
      await rpaApi.projects.deleteProject({
        robotId,
        situation,
