...
export default function useRobotOperation(homeTableRef, refreshHomeTable, refreshWithDelete) {
  ...
  async function getTableData(params) {
    const data = await rpaApi.robots.getRobotLst(params)
  ...
  async function handleDeleteRobot(editObj) {
    ...
    const data = await rpaApi.robots.isRobotInTask({ robotId })
    if (data) {
      ...
      await rpaApi.robots.deleteRobot({
        robotId,
        situation,
  ...
  async function handleRobotUpdate(record) {
    await rpaApi.robots.updateRobot({ robotId: record.robotId })
