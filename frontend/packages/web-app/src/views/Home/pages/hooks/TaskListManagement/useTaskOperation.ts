...
export function useTaskOperation() {
  ...
  async function handleDeleteTask(record) {
    ...
    await rpaApi.tasks.deleteTask({ taskId: record.taskId })
  ...
  const handleEnableTask = async (record) => {
    ...
    await rpaApi.tasks.enableTask({ taskId, enable: enable ? 1 : 0 })
    taskListTableRef.value?.fetchTableData()
  }

  const openSheculedTaskList = throttle(async (record) => {
    const { data } = await rpaApi.tasks.taskFutureTime({ task_id: record.taskId, times: 5 })
  ...
  const handleRunTask = throttle(
    (record) => {
      ...
      rpaApi.tasks.manualTrigger({ task_id: record.taskId }).then(() => {
  ...
  return {
    ...
    showQueue,
    hideQueue,
    getTableData: rpaApi.tasks.getScheduleLst,
    handleNewTask,
    handleEditTask,
    deleteTask: rpaApi.tasks.deleteTask,
    handleDeleteTask,
    handleEnableTask,
