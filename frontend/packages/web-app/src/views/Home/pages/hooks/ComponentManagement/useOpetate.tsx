...
export function useOperate(refreshTable: () => void) {
  ...
  const handleRename = (record: AnyObj) => {
    ...
    async function checkName(_rule, value: string) {
      const isExist = await rpaApi.projects.checkComponentName({ name: value, componentId })
      if (isExist) {
    ...
    async function handleConfirm(name: string) {
      await rpaApi.projects.renameComponent({ componentId, newName: name })
      refreshTable()
  ...
  const handleDelete = (record: AnyObj) => {
    const onOk = async () => {
      await rpaApi.projects.deleteComponent({ componentId: record.componentId })
      refreshTable()
  ...
  const handleCopy = async (record: AnyObj) => {
    ...
    async function newComponent(name: string) {
      await rpaApi.projects.createCopyComponent({ componentId, name })
      refreshTable()
      newProjectModal.hide()
