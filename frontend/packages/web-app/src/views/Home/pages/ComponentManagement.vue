...
async function checkName(_rule, value: string) {
  const isExist = await rpaApi.projects.checkComponentName({ name: value })
  if (isExist) {
...
function createComponent() {
  newProjectModal.show({
    ...
    defaultName: () => rpaApi.projects.getDefaultComponentName(),
  ...
  const newProject = async (componentName: string) => {
    try {
      const res = await rpaApi.projects.createComponent({ componentName })
