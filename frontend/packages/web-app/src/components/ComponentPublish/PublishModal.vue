...
async function initForm() {
  const [detail, nextVersion] = await Promise.all([
    rpaApi.projects.getComponentDetail({ componentId: props.componentId }).then(r => r.data),
    rpaApi.projects.getComponentNextVersion({ componentId: props.componentId }).then(r => r.data),
  ])
...
async function handleSubmit() {
  ...
  const successRes = await rpaApi.projects.publishComponent({ ...formState, componentId: props.componentId })
