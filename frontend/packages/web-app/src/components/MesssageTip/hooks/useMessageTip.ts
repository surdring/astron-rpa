...
export function useMessageTip() {
  ...
  const getMessageList = () => {
    ...
    // 获取消息列表
    rpaApi.market.messageList({ pageNo, pageSize }).then(res => res.data).then((data) => {
      if (data) {
  ...
  const readMessage = async ({ operateResult, id, messageType, marketId }) => {
    ...
    if (operateResult !== NOREADNUM)
      return
    const data = (await rpaApi.market.setMessageReadById({ notifyId: id })).data
  ...
  const joinTeam = async (id) => {
    const data = (await rpaApi.market.acceptJoinTeam({ notifyId: id })).data
  ...
  const refuseTeam = async (id) => {
    const data = (await rpaApi.market.refuseJoinTeam({ notifyId: id })).data
