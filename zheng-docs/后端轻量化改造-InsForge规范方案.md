...
>
> **本方案已整合**：[轻量化改造方案-最终版](./轻量化改造方案-最终版.md) 的全部内容，包括完整架构映射矩阵、前端改造分析、运维对比、风险矩阵、附录代码示例等。原最终版方案不再单独维护。
...
> 本节内容整合自 [轻量化改造方案-最终版](./轻量化改造方案-最终版.md) 的架构映射矩阵，将 AstronRPA 每个组件与 InsForge 替代方案一一对应。
...
> 本节内容整合自 [轻量化改造方案-最终版](./轻量化改造方案-最终版.md) 的前端改造分析，覆盖 web-app、electron-app、browser-plugin、auth-app 等所有前端包的改动范围。
...
**API 层封装改造：**
- 改造前：`src/api/http/index.ts` 使用 axios 封装，baseURL 指向 `/api`
- 改造后：使用 `src/insforge/index.ts` 中的 InsForge SDK 客户端，直连 `:7130`

**环境变量变化：**

| 变量 | 改造前 | 改造后 |
|:---|:---|:---|
| 后端地址 | `VITE_API_BASE_URL=/api` | `VITE_INSFORGE_BASE_URL=http://172.16.100.211:7130` |
| WebSocket | `VITE_WS_URL=ws://...:32742/api/rpa-openapi/ws` | 通过 InsForge Realtime SDK 自动连接 |
| 认证 | Casdoor OAuth URL | `VITE_INSFORGE_ANON_KEY=anon_xxx` |
| 保留服务 | — | `VITE_RPA_SERVICES_URL=http://...:32742`（过渡期） |
...
| 前端模块 | 原 API 调用 | 新 SDK 调用 |
|:---|:---|:---|
| **认证登录** | `POST /api/rpa-auth/login` | `insforgeAuth.signIn()` |
| **机器人管理** | `GET /api/robot/list` | `db.robots().select('*')` |
| **项目管理** | `GET /api/robot/project/*` | `db.projects().select('*')` |
| **流程编排** | `POST /api/rpa-openapi/workflows` | `db.workflows().insert({...})` |
| **任务调度** | `GET /api/robot/task/*` | `db.tasks().select('*')` |
| **执行记录** | `GET /api/rpa-openapi/executions` | `db.executions().select('*')` |
| **文件上传** | `POST /api/resource/upload` | `storage.upload(path, file)` |
| **文件下载** | `GET /api/resource/download` | `storage.download(path)` |
| **组件市场** | `GET /api/robot/market/*` | `db.marketComponents().select('*')` |
| **实时日志** | `WS /api/rpa-openapi/ws` | `realtime.executionChannel().subscribe()` |
...
| **P1c** | Robot/Record/Atom 只读查询 | 10 | ✅ 已完成 |
| **P2** | 单一实体 CRUD | 6 | ✅ 已完成 |
| **P3** | 复杂多表操作 | 保留旧 HTTP | ✅ 确认 |
| **P4** | WebSocket/SSE/特殊处理 | 保留旧实现 | ✅ 确认 |
| **P5a** | 非 view 文件 API 迁移 | 28 | ✅ 已完成 |
...
| `@/api/market.ts` | ⏳ 待迁移 | 42 处旧 `http.` 调用，需改为 `invokeRpaFunction('rpa-market', ...)` |
| `@/api/robot.ts` | ⏳ 待迁移 | 18 处旧 `http.` 调用，需改为 `invokeRpaFunction('rpa-crud', ...)` |
| `@/api/project.ts` | ⏳ 待迁移 | 13 处旧 `http.` 调用，需改为 `invokeRpaFunction('rpa-crud', ...)` |
| `@/api/task.ts` | ⏳ 待迁移 | 16 处旧 `http.` 调用，需改为 `invokeRpaFunction('rpa-crud', ...)` |
| `@/api/atom.ts` | ⏳ 待迁移 | 7 处旧 `http.` 调用，需改为 `invokeRpaFunction('rpa-crud', ...)` |
| `@/api/component.ts` | ⏳ 待迁移 | 4 处旧 `http.` 调用，需改为 `invokeRpaFunction('rpa-crud', ...)` |
| `@/api/mail.ts` | ⏳ 待迁移 | 4 处旧 `http.` 调用，需改为 `invokeRpaFunction('rpa-crud', ...)` |
| `@/api/contract.ts` | ⏳ 待迁移 | 1 处旧 `http.` 调用，需改为 `invokeRpaFunction('rpa-crud', ...)` |
| `@/api/setting.ts` | ⏳ 待迁移 | 8 处旧 `http.` 调用，需改为 `invokeRpaFunction('rpa-crud', ...)` |
| `@/api/common.ts` | ⏳ 待迁移 | 1 处旧 `http.` 调用（反馈提交），需迁移 |
| `@/api/plugin.ts` | ⏳ 待迁移 | 3 处旧 `http.` 调用（插件市场），需迁移 |
| `@/api/permission.ts` | ⏳ 待迁移 | 1 处旧 `http.` 调用（权限检查），需迁移 |
| `@/api/login/login.ts` | ⏳ 待迁移 | 10 处旧 `http.` 调用（登录流程），需改为 InsForge Auth SDK |
| `@/api/engine.ts` | ✅ 保留(P3) | 引擎交互，不做迁移 |
| `@/api/resource.ts` | ✅ 保留(P3) | 资源管理，不做迁移 |
| `@/api/pick.ts` | ✅ 保留(P4) | picking 服务 |
| `@/api/ws.ts` | ✅ 保留(P4) | WebSocket |

**待处理任务**：`src/api/` 下非保留模块共约 **120+ 处旧 `http.` 调用**，需全部改为使用 `rpaApi` 或直接调用 InsForge SDK。按迁移量排序：`market.ts`(42) > `robot.ts`(18) > `task.ts`(16) > `project.ts`(13) > `login/login.ts`(10) > `setting.ts`(8) > `atom.ts`(7) > `component.ts`(4) > `mail.ts`(4) > `plugin.ts`(3) > `common.ts`(1) > `contract.ts`(1) > `permission.ts`(1)。
...
> 本节内容整合自 [轻量化改造方案-最终版](./轻量化改造方案-最终版.md) 的风险矩阵，增加了结构化风险评级和紧急回滚命令。
...
> 本节内容整合自 [轻量化改造方案-最终版](./轻量化改造方案-最终版.md) 的运维对比，从资源占用、运维命令、服务管理三个维度进行比较。
...
**待处理任务**：`src/api/` 下非保留模块共约 **120+ 处旧 `http.` 调用**，需全部改为使用 `rpaApi` 或直接调用 InsForge SDK。
...
> 本节内容整合自 [轻量化改造方案-最终版](./轻量化改造方案-最终版.md) 的附录，提供改造前后代码对照、实时日志推送、定时任务迁移、MCP 工具对比等完整示例。
